          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 mt-4">
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Stories que Vendem</h1>
                  ...
                  >
                    ...
                    <div className="space-y-4 relative z-10">
                      {fw.stories.map((story, j) => {
                        ...
                        return (
                          <div key={j} className="flex items-start gap-4 p-4 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 group/story hover:bg-slate-50 dark:hover:bg-white/10 transition-colors shadow-sm">
                            ...
                            <CopyButton text={fullText} variant="icon" className="opacity-0 group-hover/story:opacity-100 transition-opacity shrink-0 bg-black/10 dark:bg-white/10 hover:bg-[#0ea5e9] hover:text-slate-900 dark:text-white" />
                          </div>
                        )
                      })}
                    </div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card rounded-3xl p-8 border border-[#0ea5e9]/20 bg-gradient-to-b from-[#0ea5e9]/10 to-transparent">
                ...
                <div className="space-y-4">
                  {STORIES_DICAS.slice(0, 8).map((dica, i) => (
                    <div key={i} className="flex items-start gap-4 p-4 rounded-2xl bg-white shadow-sm dark:shadow-none dark:bg-black/40 border border-slate-200 dark:border-white/10 hover:border-[#0ea5e9]/30 transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-800 dark:text-slate-300 leading-relaxed font-medium">{dica.dica}</p>
                      </div>
                    </div>
                  ))}
                </div>

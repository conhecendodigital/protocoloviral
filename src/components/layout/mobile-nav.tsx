  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="left" className="w-72 bg-slate-50 dark:bg-gradient-to-b dark:from-[#0B0F19] dark:to-black border-slate-200 dark:border-slate-800 p-0">
        <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-300/10 dark:border-slate-200 dark:border-white/10">
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={cn(
                ...
                isActive ? "bg-linear-to-r from-sky-500/20 to-violet-500/20 text-slate-900 dark:text-white" : "text-slate-800 dark:text-white/90 hover:text-slate-900 hover:bg-slate-100 dark:hover:text-white dark:hover:bg-white/5"
              )}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 flex gap-2 border-t border-slate-300/10 dark:border-slate-200 dark:border-white/10">

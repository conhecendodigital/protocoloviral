  ...
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      ...
      >
        ...
        {/* Icon */}
        <span className={cn('material-symbols-outlined text-[22px] shrink-0', isActive && 'fill-1')}>
          {item.icon}
        </span>
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
        {NAV_SECTIONS.map(section => (
          <div key={section.id} className="mb-1">
            ...
            {/* Collapsible section with its items */}
            {section.collapsible ? (
              <div>
                {/* Toggle button */}
                <button
                ...
                >
                  <span className="material-symbols-outlined text-[22px] shrink-0">{section.icon}</span>
                  <span className={cn(
                  ...
                  {hovered && (
                    <span className={cn('material-symbols-outlined text-[16px] shrink-0 transition-transform duration-200', expandedSections[section.id] ? 'rotate-180' : '')}>
                      expand_more
                    </span>
                  )}
                  {/* Tooltip */}
      ...
      <div className="px-2 pb-4 space-y-2 shrink-0">
        ...
        {/* User */}
        <div className="flex items-center gap-2.5 px-1 py-1 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 transition-colors cursor-pointer group/user">

...
async function run() {
  const { data: creds, error: err } = await supabase.from('creditos_mensais').select('user_id').limit(1);
  
  if (err || !creds || creds.length === 0) {
    console.error('No users found in creditos_mensais', err);
    return;
  }
